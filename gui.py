...
# Create the Dash app using the shared server
server = get_server()
...
def create_gui_app(server):
    ...
    app.layout = dbc.Container([
        ...
        *[
            dbc.Row([
                dbc.Label(f"{agent}", className="mt-2"),
                dcc.Slider(id=f"weight-{agent}", min=0, max=1, step=0.01,
        ...
        # Add a link to the simulation view
        html.Div([
    ...
    def update_sarsa_inputs(val):
        if val > 0:
            return [
                html.H5("SARSA Parameters"),
                dbc.Label("Epsilon"), dcc.Input(id='sarsa-epsilon', type='number', value=0.1, step=0.01),
                dbc.Label("Alpha"), dcc.Input(id='sarsa-alpha', type='number', value=0.1, step=0.01),
                dbc.Label("Gamma"), dcc.Input(id='sarsa-gamma', type='number', value=0.95, step=0.01)
            ]
        return []

    @app.callback(
        Output('moody-config', 'children'),
        Input('weight-MoodySARSAAgent', 'value')
    )
    def update_moody_inputs(val):
        if val > 0:
            return [
                html.H5("Moody SARSA Parameters"),
                dbc.Label("Epsilon", style={"margin-right": "15px"}), dcc.Input(id='moody-epsilon', type='number', value=0.1, step=0.01, style={"margin-right": "15px"}),
    ...
    Input('overview-graph', 'tapNodeData'),         # Renamed input
    State('num-nodes', 'value'),
    State('overview-graph', 'elements')             # Renamed state
    )
    ...
    Input('overview-graph', 'mouseoverNodeData'),   # Changed to a safer event
    State('overview-graph', 'elements')
    )
    def update_node_positions(node_data, elements):
        if not elements:
            return dash.no_update
        # Update our global node_positions dictionary
    ...
    def launch_simulation(n_clicks, elements, num_nodes, num_edges, num_games, betrayal_thresh, reconnect_pct,
        ...
        # Pass the path to this module to test3.py
        # p = subprocess.Popen([sys.executable, "test3.py", 
        #                 "--config", "params.json"])
